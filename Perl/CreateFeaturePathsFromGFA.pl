...
while(my $line = <F>){
		...
		my $end = $infos[4];
		my $feature = $infos[2];
		if ($pattern eq 'BGC' or $pattern eq 'Defense system'){
		       	if ($feature =~/$pattern/ && $infos[8] =~/ID=([^;]+);/){
				my $feature_id = $1;
				$coordinates{$chr}.= "$feature_id:$start-$end|";
				$allgenes{$feature_id}=1;
				for (my $i = $start; $i <= $end; $i++){
					$coding_positions{$chr}{$i} = "$feature_id,";
				}
			}
		}
		if ($pattern eq 'antiSMASH'){
			if ($feature eq 'gene' && $source=~/antiSMASH/ && $infos[8] =~/ID=([^;]+);/){
				my $feature_id = $1;
				$coordinates{$chr}.= "$feature_id:$start-$end|";
				$allgenes{$feature_id}=1;
				for (my $i = $start; $i <= $end; $i++){
